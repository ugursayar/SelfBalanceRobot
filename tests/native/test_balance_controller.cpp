...
static void test_reset_clears_integral_and_previous_error() {
  BalanceController controller;
  controller.setTunings(0.0f, 1.0f, 1.0f);
  ...
  assert(controller.integral_ == 0.0f);
  assert(controller.previousError_ == 0.0f);
  assert(!controller.hasPreviousError_);
...
static void test_small_errors_ramp_proportional_gain() {
  BalanceController controller;
  ...
  assert(controller.update(-1.0f, 0.02f) == 20);
  assert(controller.update(-2.0f, 0.02f) == 60);
...
int main() {
  ...
  test_small_errors_ramp_proportional_gain();
